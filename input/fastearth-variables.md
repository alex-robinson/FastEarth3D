...
The Maxwell memory-stress fields `tau_*` are the prognostic state restored on
restart; the reference fields `z_bed_eq`/`h_ice_ref` are static (written once) and
checked on read; the rest are diagnostic.
